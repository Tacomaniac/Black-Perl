...
function XPerl_RaidMonitor_Init(self)
	...
	function cast:HealthTotals()
		...
		if IsInRaid() then--actually check if we are in a raid
			for i = 1, GetNumGroupMembers() do
				...
				if (UnitIsConnected(id)) then
					...
					if IsRetail then 
						local percent = UnitHealthPercent(id, true, CurveConstants.ScaleTo100)
					else
		...
		if IsRetail then 
			local totalHealth = UnitHealthPercent(id, true, CurveConstants.ScaleTo100)
		else
		...
		if totalHealth ~= 0 then
			if IsRetail then
				ScaleBarColour(self.totals.RaidHealth, totalHealth) 
			else
