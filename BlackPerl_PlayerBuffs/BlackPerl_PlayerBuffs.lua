...
function XPerl_PlayerBuffs_Update(self)
	...
	if slot then
		...
		if filter and unit then
			...
			if IsRetail then 
				--Unit is not dead or a ghost (no reason to check)
				--If Unit is not a friendly (aka not in party or raid then do not puta dispell higlihgt up there)
				if UnitIsDeadOrGhost(unit) and not UnitIsFriend("player", unit) then
					self.border:Hide()
					return
				end
				--get the best instance of the aura that matches that is either HARMFUL or a RAID related one
				...
				if bestAuraInstanceID then
					--Get the color associated same as DebuffTypeColor
					...
					if name and filter == "HARMFUL" then
							self.border:Show()
							local borderColor = color --DebuffTypeColor[(dispelName or "none")]
			...
			elseif self.cooldown and (duration or 0) ~= 0 and conf.buffs.cooldown and (sourceUnit or conf.buffs.cooldownAny) then
				local start = expirationTime - duration
				XPerl_CooldownFrame_SetTimer(self.cooldown, start, duration, 1, sourceUnit)
				--[[if (pconf.buffs.flash) then
