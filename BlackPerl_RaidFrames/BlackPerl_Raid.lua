			...
			if IsRetail then 
				...
				if (rconf.healerMode.enable) then
					...
					elseif rconf.precisionPercent then
						self.statsFrame.healthBar.text:SetFormattedText(perc1F, percentHp)
					else
	...
	if (rconf.mana) then
		...
		if (rconf.manaPercent and XPerl_GetDisplayedPowerType(partyid) == 0 and not self.pet) then
			if (rconf.values) then -- TODO rconf.manavalues
				...
				if IsRetail then
					local pmanaPct = UnitPowerPercent(partyid, pType, true, CurveConstants.ScaleTo100)
					if rconf.precisionManaPercent then
						self.statsFrame.manaBar.text:SetFormattedText(perc1F, pmanaPct)
					else
						self.statsFrame.manaBar.text:SetFormattedText(percD, pmanaPct)
					end
				else
					--Begin 4.3 division by 0 work around to ensure we don't divide if max is 0
					local pmanaPct
