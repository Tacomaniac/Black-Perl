...
local function smoothColor(percentage)
	...
	----i am here dont forget :) gotta fix the color without doing math on the percent
	--Set color to black to see what happens
		r = 0
		g = 0 
		b = 0
...
function XPerl_SetSmoothBarColor(self, percentage)
	if (self) then
		...
		-- end
			...
			local c = conf.colour.bar
			r = min(1, max(0, c.healthEmpty.r + (c.healthFull.r - c.healthEmpty.r)))
			g = min(1, max(0, c.healthEmpty.g + (c.healthFull.g - c.healthEmpty.g)))
			b = min(1, max(0, c.healthEmpty.b + (c.healthFull.b - c.healthEmpty.b)))
					...
					local isPlayer
						...
						if UnitInParty(self.partyid) or UnitInRaid(self.partyid) then
							if  (self.conf.buffs.bigpet) then
								--currently will only tell you if its a player in a part or raid
								isPlayer = UnitIsPlayer(self.partyid) 
								--unitCaster == "player" or unitCaster == "pet" or unitCaster == "vehicle"
							else
...
function XPerl_SetExpectedAbsorbs(self)
	...
	if IsRetail then
			----Need to figure out the logic for this --currently commented out :)
		return
	end
...
function XPerl_SetExpectedHealth(self)
	...
	if (bar) then
		...
		if (not UnitIsDeadOrGhost(unit)) then
			...
			if IsRetail then 
				--changed to this instead of below because of addition to health which is no longer allowed
				--bar:SetValue(UnitGetIncomingHeals(unit)) --Overlay bar with amount of heals incoming (not really sure how much this will help)
			else
